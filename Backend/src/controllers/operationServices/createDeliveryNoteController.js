...
export const createDeliveryNoteController = async (req, res, next) => {
  ...
  try {
    ...
    const { sale_id, deliverer_id, delivery_status, address_id, product_id } = req.body;

    // Verificar si el producto tiene stock suficiente
    const [product] = await pool.query(`SELECT stock FROM Products WHERE id_product = ?`, [product_id]);
    const productStock = product[0]?.stock;

    if (productStock <= 0) {
      // Stock insuficiente, devuelve un error
      res.status(400).json(errorResponse({ message: 'Stock insuficiente para el producto' }));
      return;
    }

    // Insertar la nota de entrega con los datos proporcionados
    const noteId = await insertDeliveryNote(sale_id, deliverer_id, delivery_status, address_id, product_id);
