...
export async function loadMinusData(db) {
  try {
    ...
    console.log(chalk.bold.blue(`->✏️ Insertando 100 productos en la tabla Products...`));
    ...
    for (let i = 0; i < 80; i++) {
      const saleProduct = {
        ...
        product_id: productData[i % 100].id_product,
    ...
    for (let i = 0; i < 80; i++) {
      ...
      const saleDate = i < 20 ? faker.date.recent() : faker.date.past({ days: 300 });
      const operationStatus = i < 20 ? 'open' : 'closed';
    ...
    await db.query(
      `INSERT INTO Sales (id_sale, ref_SL, user_id, saleProduct_id, customer_id, operation_status, create_at) VALUES ?`,
    ...
    for (let i = 0; i < 80; i++) {
      ...
      const deliveryStatus = i < 20 ? 'pending' : 'delivered';

      const deliveryNote = {
        id_note: faker.string.uuid(),
        ref_DN: currentRefDeliveryNote,
        sale_id: salesData[i].id_sale,
        deliverer_id: userData[i % 30].id_user,
        delivery_status: deliveryStatus,
        customer_id: customerData[i % 80].id_customer,
        address_id: addressData[i % 200].id_address,
        saleProduct_id: salesProductData[i].id_saleProduct,
    ...
    for (let i = 0; i < 80; i++) {
      ...
      const shipmentStatus = i < 20 ? 'pending' : 'delivered';
    ...
    for (let i = 0; i < 80; i++) {
      let totalPrice = productData[i % 100].price * salesProductData[i].quantity;
      ...
      const invoiceStatus = i < 20 ? 'pending' : faker.helpers.arrayElement(['paid', 'overdue', 'cancelled', 'refunded', 'disputed', 'sent']);

      const invoice = {
        id_invoice: faker.string.uuid(),
        ref_IN: currentRefInvoice,
        agentUser_id: userData[i % 30].id_user,
        sale_id: salesData[i].id_sale,
        customer_id: customerData[i % 80].id_customer,
        company_name: customerData[i % 80].company_name,
        NIF: customerData[i % 80].NIF,
        address: addressData[i % 200].address,
    ...
    for (let i = 0; i < 80; i++) {
      ...
      const paymentStatus = i < 20 ? 'pending' : 'paid';
    ...
    for (let i = 0; i < 130; i++) {
      ...
      if (i < 80) {
        ...
        const invoice = invoiceData.find(inv => inv.sale_id === sale_id);
        if (invoice) {
          invoice_id = invoice.id_invoice;
          agentUser_id = invoice.agentUser_id;
        }
    
        const payment = paymentData.find(pay => pay.invoice_id === invoice_id);
        if (payment) {
          payment_id = payment.id_payment;
        }
    
        const deliveryNote = deliveryNoteData.find(dn => dn.sale_id === sale_id);
        if (deliveryNote) {
          deliveryNote_id = deliveryNote.id_note;
          deliveryUser_id = deliveryNote.deliverer_id;
        }
    
        const shipment = shipmentData.find(sh => sh.deliveryNote_id === deliveryNote_id);
        if (shipment) {
          shipment_id = shipment.id_shipment;
        }
      } else {
        service_type = 'visit';
        const visitIndex = i - 80;
        if (visitData[visitIndex]) {
          visit_id = visitData[visitIndex].id_visit;
          const user = await selectUserByIdVisit(visit_id);
          agentUser_id = user ? user.user_id : null;
        }
      }
