...
export const selectVisitSearchModel = async (searchTerm) => {
  ...
  const [rows] = await pool.query(
    `SELECT Visits.id_visit, Visits.ref_VT, Customers.name AS customer_name, Visits.visit_date, Visits.observations, Visits.rating_comment, Visits.visit_status, Visits.rating_visit
    FROM Visits
